...
let plant = {
  ...
  events: () => {
    ...
    const tattyOldRecipeButton = document.getElementById('tatty-old-recipe');
    ...
    var timewarp = new Recipe('timewarp', .00208, 3, 0.5, 30);
    var filsToms = new Recipe('filsToms', 30, 5, 10, 20);
    var disco = new Recipe('disco', 30, 0.5, 1, 30);
    var temp = new Recipe('temp', 30, 15, 15, 10); // toggle light visuals
    ...
    tattyOldRecipeButton.addEventListener('click', function () {
      connectPlantIO().then((results, anotherval) => {
        tattyOldRecipeButton.innerHTML = "Connected to CloudPlantIO!";
